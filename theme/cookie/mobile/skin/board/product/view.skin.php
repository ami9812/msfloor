...
if($v_img_count) {
    ...
    for ($i=0; $i<=count($view['file']); $i++) {
        if ($view['file'][$i]['view']) {
            ...
            //echo ((get_view_thumbnail($view['file'][$i]['view'])) % 2) ? '' : '<br>';
            //echo ($i % 2) ? '' : '<br>';
...
<article id="bo_v">
    ...
    <section id="bo_v_atc">
        ...
        <div id="bo_v_con">
        <?php 
        $in_html = "<img><a>"; // 제거하지 말아야 할 html 태그
        echo strip_tags(get_view_thumbnail($view['content']), $in_html); // 콘텐츠에서 in_html 에 정의된 태그를 제외한 모든 태그 제거 
        ?>
        </div>
