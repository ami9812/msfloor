...
if ($g5['title'] == "갤러리"){
    ...
    echo '<li><a href="'.G5_BBS_URL.'/board.php?bo_table=gallery" id="bo_cate_on">갤러리</a></li>';
    echo '</ul>';
    echo '</nav>';
}elseif ($is_category) { ?>
<nav id="bo_cate">
    ...
    if(preg_match("/\.({$config['cf_image_extension']})$/i", $file[0]['file'])) {
        ...
        <li class="gall_li <?php if ($wr_id == $list[$i]['wr_id']) { ?>gall_now<?php } ?>">
           <!-- <a href="<?php //echo $list[$i]['href'] ?>" class="gall_img">-->
            <div class="install"><a href="<?php echo $file_src; ?>" class="gall_img">
            <?php
    ...
    <?php } ?>
        ...
        <script>    
            (function($) { // function 하나에 기존 코드들을 모두 묶어줍니다.
                ...
                $(function(){
                    var $gallery = $('.install a').simpleLightbox();
