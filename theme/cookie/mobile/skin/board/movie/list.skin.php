...
if ($g5['title'] == "시연 동영상"){
    ...
    echo '<li><a href="'.G5_BBS_URL.'/board.php?bo_table=movie" id="bo_cate_on">시연 동영상</a></li>';
    echo '</ul>';
    echo '</nav>';
}elseif ($is_category) { ?>
<nav id="bo_cate">
...
<div id="bo_gallery">
    ...
    ?>
        ...
        <li class="gall_li">
            <a href="<?php echo ($is_admin ? $list[$i]['href'] : $list[$i]['wr_link1']); ?>" class="gall_img <?php echo ($is_admin ? '' : 'popup-youtube'); ?>">
            <?php
            // 썸네일. (유투브 코드로 썸네일처리)
            $thumbsrc = '';
            ...
            if($thumbsrc) {
                $img_content = '<img src="'.$board_skin_url.'/img/youtube.png" style="position:absolute; width:30px; margin:15px 20px;"><img src="'.$thumbsrc.'" alt="'.$list[$i]['subject'].'" width="'.$thumb_width.'" height="'.$thumb_height.'">';
            } else {
            ...
            <div class="gall_txt">
                ...
                ?>
                <?php } ?>
                <a href="<?php echo $list[$i]['wr_link1'] ?>" class="gall_li_tit <?php echo ('popup-youtube'); ?>">
